...
function CalendarPreview({ phases, startDate }: { phases: Phase[], startDate: string }) {
  ...
  const c = { background: '#111111', border: '1px solid rgba(184,151,106,0.12)', borderRadius: '8px' }
  ...
  return (
    <div style={c}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid rgba(184,151,106,0.12)' }}>
        <button onClick={() => setViewDate(new Date(year, month-1, 1))} style={{ background: 'none', border: '1px solid rgba(184,151,106,0.2)', borderRadius: '4px', color: '#b8976a', cursor: 'pointer', width: '26px', height: '26px', fontSize: '14px' }}>‹</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', borderBottom: '1px solid rgba(184,151,106,0.08)' }}>
        {DAYS.map(d => <div key={d} style={{ padding: '6px', textAlign: 'center', fontSize: '9px', textTransform: 'uppercase', letterSpacing: '1px', color: '#4a4030' }}>{d}</div>)}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))' }}>
        {cells.map((day, idx) => {
          ...
          const c2 = COLORS.find(c => c.bg === phase?.color)
          return (
            <div key={idx} style={{ minHeight: '70px', borderRight: '1px solid rgba(184,151,106,0.05)', borderBottom: '1px solid rgba(184,151,106,0.05)', padding: '3px', background: isWeekend ? 'rgba(255,255,255,0.01)' : 'transparent' }}>
              {day && (
                <>
                  <div style={{ width: isToday ? '18px' : 'auto', height: isToday ? '18px' : 'auto', borderRadius: isToday ? '50%' : 0, background: isToday ? '#b8976a' : 'transparent', color: isToday ? '#0a0a0a' : isWeekend ? '#3a3020' : '#4a4030', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2px', marginLeft: '2px' }}>{day}</div>
                  {phase && (
                    <div style={{ background: phase.color, color: c2?.text ?? '#fff', borderRadius: '2px', padding: '1px 4px', fontSize: '9px', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ ...card, overflow: 'hidden', marginBottom: '16px' }}>
            ...
            {phases.map((phase, i) => (
              <div
                ...
                onDrop={() => handleDrop(i)}
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', borderBottom: '1px solid rgba(184,151,106,0.08)', cursor: 'grab', background: dragOver === i ? 'rgba(184,151,106,0.05)' : 'transparent', borderTop: dragOver === i ? '2px solid #b8976a' : '2px solid transparent' }}
              >
                <div style={{ color: '#3a3020', fontSize: '14px', cursor: 'grab', flexShrink: 0 }}>⠿</div>
                <div style={{ width: '4px', height: '36px', borderRadius: '2px', background: phase.color, flexShrink: 0 }} />
                <div style={{ flex: 1, display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <input value={phase.name} onChange={e => updatePhase(i, 'name', e.target.value)} style={{ ...inputStyle, flex: 1 }} />
                  <input type="number" value={phase.duration_days} onChange={e => updatePhase(i, 'duration_days', parseInt(e.target.value) || 1)} style={{ ...inputStyle, width: '70px' }} min="1" />
                  <span style={{ fontSize: '10px', color: '#4a4030', whiteSpace: 'nowrap' }}>days</span>
                </div>
                <div style={{ display: 'flex', gap: '3px' }}>
                  {COLORS.map(c => (
                    <button key={c.bg} onClick={() => updatePhase(i, 'color', c.bg)} style={{ width: '16px', height: '16px', borderRadius: '2px', background: c.bg, border: phase.color === c.bg ? '2px solid #f5f0e8' : '2px solid transparent', cursor: 'pointer' }} />
                  ))}
                </div>
                <button onClick={() => removePhase(i)} style={{ background: 'none', border: 'none', color: '#3a3020', cursor: 'pointer', fontSize: '16px', padding: '2px' }}>×</button>
              </div>
            ))}
